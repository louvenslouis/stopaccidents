...
export type NearbyEvent = SafetyReportSummary & {
  event_id: string;
  distance_m: number;
...
export async function nearbyEvents(
  ...
  const query = supabase.rpc("nearby_report_events", {
    p_kind: kind,
    p_latitude: draft.coordinates.latitude,
    p_longitude: draft.coordinates.longitude,
  ...
  if (error)
    throw new Error(
      "Impossible de vérifier les événements proches. Réessayez.",
    );
...
export async function prepareReportEvent(kind: ReportKind, draft: EventDraft) {
  ...
  if (error)
    throw new Error(
      error.code === "22023"
        ? "Cet événement n’est plus proposé. Fermez puis reprenez le formulaire pour actualiser."
        : "Le rattachement à l’événement n’a pas pu être confirmé. Réessayez.",
    );
}
export async function readReportEvent(
  kind: ReportKind,
  id: string,
  signal?: AbortSignal,
): Promise<ReportEvent | null> {
  const query = supabase.rpc("read_report_event", {
    p_kind: kind,
    p_report_id: id,
  });
  const { data, error } = await (signal ? query.abortSignal(signal) : query);
