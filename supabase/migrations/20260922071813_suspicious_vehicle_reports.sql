create table public.suspicious_vehicle_reports (
  id uuid primary key,
  ...
  vehicle_description text not null default '' check (char_length(vehicle_description) <= 1500),
  observed_behavior text not null default '' check (char_length(observed_behavior) <= 1000),
  ...
  constraint suspicious_vehicle_location_required check (char_length(trim(location_description)) >= 3 or latitude is not null),
  constraint suspicious_vehicle_coordinate_pair check ((latitude is null) = (longitude is null)),
  constraint suspicious_vehicle_accuracy_requires_coordinates check (latitude is not null or location_accuracy_m is null),
  constraint suspicious_vehicle_vehicle_description_after_step_two check (completed_step < 2 or char_length(trim(vehicle_description)) >= 3),
  constraint suspicious_vehicle_observed_behavior_after_step_two check (completed_step < 2 or char_length(trim(observed_behavior)) >= 3)
);

create index suspicious_vehicle_reports_reporter_idx on public.suspicious_vehicle_reports(reporter_id, created_at desc);
create index suspicious_vehicle_reports_triage_idx on public.suspicious_vehicle_reports(status, created_at desc);

alter table public.suspicious_vehicle_reports enable row level security;
revoke all on public.suspicious_vehicle_reports from anon, authenticated;
grant select on public.suspicious_vehicle_reports to authenticated;
grant insert (
  id, location_description, latitude, longitude, location_accuracy_m,
  vehicle_description, observed_behavior, details, completed_step
) on public.suspicious_vehicle_reports to authenticated;
grant update (
  location_description, latitude, longitude, location_accuracy_m,
  vehicle_description, observed_behavior, details, completed_step, updated_at
) on public.suspicious_vehicle_reports to authenticated;

create policy suspicious_vehicle_reports_read_own
  on public.suspicious_vehicle_reports for select to authenticated
  using (reporter_id = (select auth.uid()));
create policy suspicious_vehicle_reports_insert_own
  on public.suspicious_vehicle_reports for insert to authenticated
  with check (reporter_id = (select auth.uid()));
create policy suspicious_vehicle_reports_update_own
  on public.suspicious_vehicle_reports for update to authenticated
  using (reporter_id = (select auth.uid()))
  with check (reporter_id = (select auth.uid()));

create function public.save_suspicious_vehicle_report_step(
  p_id uuid,
  p_step integer,
...
begin
  ...
  if p_step = 1 then
    ...
    if char_length(trim(coalesce(p_vehicle_description, ''))) < 3 then
      raise exception 'Vehicle description required' using errcode = '23514';
    end if;
    if char_length(trim(coalesce(p_observed_behavior, ''))) < 3 then
      raise exception 'Observed behavior required' using errcode = '23514';
    end if;
    update public.suspicious_vehicle_reports
      set vehicle_description = trim(p_vehicle_description),
          observed_behavior = trim(p_observed_behavior),
...
revoke all on function public.save_suspicious_vehicle_report_step(
  uuid, integer, text, double precision, double precision, double precision,
  text, text, text
) from public, anon, authenticated;
grant execute on function public.save_suspicious_vehicle_report_step(
  uuid, integer, text, double precision, double precision, double precision,
  text, text, text
) to authenticated;

comment on table public.suspicious_vehicle_reports is 'Unverified suspicious vehicle reports. Only authors may modify rows; public readers use explicit projections without reporter identity.';
create index suspicious_vehicle_reports_latest_idx on public.suspicious_vehicle_reports(created_at desc, id desc);
...
create function private.read_suspicious_vehicle_report(p_id uuid)
...
$$;
revoke all on function private.read_suspicious_vehicle_report(uuid) from public, anon, authenticated;
grant execute on function private.read_suspicious_vehicle_report(uuid) to anon, authenticated;

create function public.read_suspicious_vehicle_report(p_id uuid)
returns jsonb language sql stable security invoker set search_path = '' as $$
  select private.read_suspicious_vehicle_report(p_id);
$$;
revoke all on function public.read_suspicious_vehicle_report(uuid) from public, anon, authenticated;
grant execute on function public.read_suspicious_vehicle_report(uuid) to anon, authenticated;




comment on function public.read_suspicious_vehicle_report(uuid) is 'Unverified suspicious vehicle report, without reporter identity.';
